...
static const char *model_bin;
static uint32_t model_bin_len;
static const char *tokenizer0_bin;
static float temperature = 1.0f;
static float topp = 0.9f;
static int steps = 256;
static const char *prompt;
static unsigned long long rng_seed = 42;
...
static void malloc_run_state(RunState* s, Config* p) {
    ...
    // These are too large for SRAM
    s->key_cache = (float *)pcalloc(p->n_layers * p->seq_len * kv_dim, sizeof(float));
    s->value_cache = (float *)pcalloc(p->n_layers * p->seq_len * kv_dim, sizeof(float));
...
static void read_checkpoint(Config* config, TransformerWeights* weights,
    ...
    *file_size = model_bin_len; // get the file size, in bytes
...
void __not_in_flash_func(matmul)(float* xout, float* x, float* w, int n, int d) {
    // W (d,n) @ x (n,) -> xout (d,)
...
static void build_tokenizer(Tokenizer* t, int vocab_size) {
    // i should have written the vocab_size into the tokenizer file... sigh
    t->vocab_size = vocab_size;
    // malloc space to hold the scores and the strings
    t->vocab = (const char**)pmalloc(vocab_size * sizeof(char*));
    t->vocab_scores = (float*)pmalloc(vocab_size * sizeof(float));
    ...
    // read in the file
    const char *file = tokenizer0_bin;
    memcpy(&t->max_token_length, file, sizeof(int)); file += sizeof(int);
    int len;
    for (int i = 0; i < vocab_size; i++) {
        memcpy(t->vocab_scores + i, file, sizeof(float)); file += sizeof(float);
        memcpy(&len, file, sizeof(int)); file += sizeof(int);
        t->vocab[i] = file; file += len + 1; //(char *)pmalloc(len + 1);
    }
}
...
static void encode(Tokenizer* t, const char *text, int8_t bos, int8_t eos, int *tokens, int *n_tokens) {
    ...
    if (t->sorted_vocab == NULL) {
        ...
        t->sorted_vocab = (TokenIndex*)pmalloc(t->vocab_size * sizeof(TokenIndex));
...
static void build_sampler(Sampler* sampler, int vocab_size, float temperature, float topp, unsigned long long rng_seed) {
    ...
    sampler->probindex = (ProbIndex*)pmalloc(sampler->vocab_size * sizeof(ProbIndex));
}
...
// ----------------------------------------------------------------------------
// generation loop
static void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, const char *prompt, int steps) {
